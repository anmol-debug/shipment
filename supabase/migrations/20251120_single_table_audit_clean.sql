-- Migration: Single-table audit and versioning system (CLEAN VERSION)
-- This replaces the two-table design with a single shipment_history table

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Users can view history for accessible shipments" ON "public"."shipment_history";
DROP POLICY IF EXISTS "System can insert history" ON "public"."shipment_history";

-- Drop old tables if they exist (CASCADE removes dependent objects)
...
DROP FUNCTION IF EXISTS "public"."create_history_entry"(uuid, text, uuid, text, text, jsonb, jsonb) CASCADE;
...
DROP FUNCTION IF EXISTS "public"."get_field_changes"(uuid, integer, integer) CASCADE;
...
CREATE INDEX "idx_history_shipment_version_desc"
    ON "public"."shipment_history" ("shipment_id", "version_no" DESC);

CREATE INDEX "idx_history_shipment_timestamp_desc"
    ON "public"."shipment_history" ("shipment_id", "timestamp" DESC);

CREATE INDEX "idx_history_actor"
    ON "public"."shipment_history" ("actor_id");

CREATE INDEX "idx_history_event_type"
...
CREATE FUNCTION "public"."get_next_version_no"(p_shipment_id uuid)
RETURNS integer
LANGUAGE sql
STABLE
AS $$
...
CREATE FUNCTION "public"."create_history_entry"(
    p_shipment_id uuid,
    p_event_type text,
    p_actor_id uuid,
    p_actor_name text,
...
CREATE FUNCTION "public"."restore_shipment_version"(
    p_shipment_id uuid,
...
CREATE FUNCTION "public"."get_field_changes"(
    p_shipment_id uuid,
